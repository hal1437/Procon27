...
Problem ProblemMaker::MakeTriangleProblem(const Range<int> edge_div_range,
                                          const int        edge_div_round,
                                          const int        density       ,
	...
	do{
		...
		for(int v=0;v<4;v++){
			...
			//4~6回等分
			int times;
			if(edge_div_range.diff()>0)times = rd() % edge_div_range.diff() + edge_div_range.min;
			else times = edge_div_range.min;
			
			if(times == 1){
				pairs.push_back(std::make_pair(joint_cc,(joint_cc+1)%4));
				joint_cc++;
			}else{
